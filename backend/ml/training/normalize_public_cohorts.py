...
def folha(caminho):
    ...
    def idx(c):
        n = 0
        for ch in re.match(r"[A-Z]+", c).group():
            n = n * 26 + ord(ch) - 64
        return n - 1
...
def hereford(p):
    ...
    for i, r in enumerate(folha(p)):
        ...
        if None in (w, wh, cd, iw, hg):
            continue
        linhas.append(
            dict(
                dataset_source="CowDatabase",
                external_animal_id=str(int(num(r[0]))),
                breed="Hereford",
                sex="female",
                age_months="",
                body_length_cm=obl,
                withers_height_cm=wh,
                thoracic_depth_cm=cd,
                rump_width_cm=iw,
                chest_girth_cm=hg,
                real_weight_kg=w,
...
def angus(p, fonte, raca, sexo, col_peso, nota):
    ...
    linhas = []
    for r in folha(p):
        if len(r) < max(15, col_peso + 1):
            continue
        n = num(r[0])
        if n is None or not (1 <= n <= 200):
            continue
        wh, hh, cw, hg = (
            [num(r[j]) for j in (11, 12, 13, 14)]
            if fonte == "CowDatabase2"
...
def acl(p):
    ...
    for r in csv.DictReader(open(p)):
        linhas.append(
            dict(
                dataset_source="ACL-TP-03/2022",
                external_animal_id=r["external_animal_id"],
                breed="Limousine",
                sex="male",
                age_months=r["age_months"],
                body_length_cm="",
                withers_height_cm=r["withers_height_cm"],
                thoracic_depth_cm="",
                rump_width_cm=r["rump_width_cm"],
                chest_girth_cm=r["chest_girth_cm"],
...
if __name__ == "__main__":
    ...
    saida = Path("backend/ml/datasets/processed")
    saida.mkdir(parents=True, exist_ok=True)
    conjuntos = [
        ("cowdatabase_hereford_103.csv", hereford(D / "Measurements.xlsx")),
        (
            "cowdatabase2_angus_96.csv",
            angus(
                D / "Database2022v2 (1).xlsx",
                "CowDatabase2",
                "Aberdeen Angus",
                "female",
                10,
                "Voronezh cohort; 4 measurements only",
            ),
        ),
        (
            "cowdatabase3_angus_93.csv",
            angus(
                D / "CowDatabase3_Database2023v2.xlsx",
                "CowDatabase3",
                "Aberdeen Angus",
                "female",
                13,
                "heifers, fattening; weight of 11/10/2022; NOT for training, weight CV 3.43%",
            ),
        ),
